...
from ml.training.save import save_model_history
from ml.training.load import load_model_history
...
def test_persistence_equivalence():
    ...
    print("Target:", training_result["target_name"])
    print("Algorithm:", training_result["algorithm"])
    print("Training rows:", training_result["training_rows"])
    print("Test rows:", training_result["test_rows"])
    print(
        "Feature count:",
        len(training_result["feature_names"])
    )
    ...
    print("Model history ID:", model_history_id)
    ...
    loaded_result = load_model_history(model_history_id)
    ...
    print(
        "Loaded target:",
        loaded_result["target_name"]
    )

    print(
        "Loaded feature count:",
        len(loaded_result["feature_names"])
    )
    ...
    print("Restored predictions:", restored_predictions)
    ...
    for index, (original, restored) in enumerate(
        zip(original_predictions, restored_predictions)
    ):
        if not isclose(
            original,
            restored,
            ...
            differences.append(
                {
                    ...
                    "original": original,
                    "restored": restored,
                    "difference": abs(original - restored),
                }
            )
    ...
    original_features = training_result["feature_names"]
    restored_features = loaded_result["feature_names"]
    ...
    if loaded_result["model_history_id"] != model_history_id:
        raise AssertionError(
    ...
    print()
    print("==================================================")
    print("     MODEL PERSISTENCE EQUIVALENCE PASSED")
    print("==================================================")
